...
from main_shop.models import ProductSize
    ...
    def post(self, request):
        ...
        if not payment_provider or payment_provider not in ['stripe', 'heleket']:
            context = {
                ...
                'cart_items': cart.items.select_related('product', 'product_size__size').order_by('added_at'),
                'total_price': cart.subtotal,
                'eror_message': 'Please select a valid provider (Stripe or Heleket)',
            }
            if request.headers.get('HX-Request'):
                return TemplateResponse(request, 'orders/checkout_content.html', context)
        ...
        if not from_data.get('email'):
            from_data['email'] = request.user.mail
        form = OrderForm(from_data, user=request.user)
        ...
        if form.is_valid():
            ...
            try:
                ...
                context = {
                    ...
                    'cart_items': cart.items.select_related('product', 'product_size__size').order_by('added_at'),
                    'total_price': total_price,
                    'error_message': f'Payment proccesing error: {str(e)}'
                }
            ...
            context = {
                    ...
                    'cart_items': cart.items.select_related('product', 'product_size__size').order_by('added_at'),
                    'total_price': total_price,
                    'error_message': f'Please corect the errors on the form.'
                }
            if request.headers.get('HX-Request'):
                    return TemplateResponse(request,'orders/checkout_content.html', context)
            return render(request, 'orders/checkout.html', context)
